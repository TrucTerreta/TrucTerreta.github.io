...
export async function dealHand(){
  await mutate(state=>{
    ...
    state.hand=Logica.makeHand(state.mano);state.status='playing';
...
export async function playCard(card){
  ...
  try{
    await mutate(state=>{
      ...
      if(alreadyPlayed(h,session.mySeat)){console.warn('PLAYCARD: already played');return false;}
      ...
      h.envitAvailable=false;
      const plrName=state.players?.[K(session.mySeat)]?.name||`J${session.mySeat}`;
    ...
    setTimeout(()=>{
      ui.locked=false;
      if(session.roomRef){
        get(session.roomRef).then(snap=>{if(snap.val())_renderAll(snap.val());}).catch(()=>{});
      }
    },380);
  }
}
...
export async function goMazo(){
  await mutate(state=>{
    ...
    if(h.turn!==session.mySeat||h.mode!=='normal'||h.pendingOffer)return false;
...
export async function startOffer(kind){
  await mutate(state=>{
    ...
    if(kind==='envit' || kind==='falta'){
      ...
      h.resume={mode:h.mode, turn:h.turn, oldOffer: h.pendingOffer || null}; 
      h.pendingOffer={kind:'envit', level:level, by:session.mySeat, to:other(session.mySeat)};
      h.mode='respond_envit';
      h.turn=other(session.mySeat);
      
      const nom = state.players?.[K(session.mySeat)]?.name || `J${session.mySeat}`;
    ...
    if(kind==='truc'){
      ...
      h.resume={mode:h.mode, turn:h.turn, oldOffer: h.pendingOffer || null};
      h.pendingOffer={kind:'truc', level:trucLevel, by:session.mySeat, to:other(session.mySeat)};
      h.mode='respond_truc';
      h.turn=other(session.mySeat);
...
export async function respondEnvit(choice){
  await mutate(state=>{
    ...
    if(!h||state.status!=='playing'||h.status!=='in_progress')return false;
    if(!offer||offer.kind!=='envit'||h.turn!==session.mySeat||h.mode!=='respond_envit')return false;
    const caller=offer.by,resp=offer.to;
    if(choice==='vull'){
      h.envit={state:'accepted',caller,responder:resp,acceptedLevel:offer.level,acceptedBy:session.mySeat};
...
export async function respondTruc(choice){
  await mutate(state=>{
    const h=state.hand,offer=h?.pendingOffer;
    if(!h||state.status!=='playing'||h.status!=='in_progress')return false;
    if(!offer||offer.kind!=='truc'||h.turn!==session.mySeat||h.mode!=='respond_truc')return false;
    const caller=offer.by,resp=offer.to;
    if(choice==='vull'){
      h.truc={state:'accepted',caller,responder:resp,acceptedLevel:offer.level,acceptedBy:session.mySeat};
